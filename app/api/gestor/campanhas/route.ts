...
export async function GET(req: NextRequest) {
  ...
  try {
    // Agrega por campanha somando métricas do período (não só o último snapshot)
    const { rows } = await pool.query(
      `SELECT
         ...
         -- Status da snapshot mais recente
         (SELECT status FROM campaign_snapshots s2
         ...
         MAX(COALESCE(result_label, 'Resultados')) AS result_label,
         MAX(COALESCE(result_type, ''))            AS result_type,
         -- Métricas derivadas calculadas dos totais (não média de médias)
